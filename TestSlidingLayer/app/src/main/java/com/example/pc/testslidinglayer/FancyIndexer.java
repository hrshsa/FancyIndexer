...
public class FancyIndexer extends View {
    ...
    float mWidthOffset = 20.0f;

    // 最小字体大小
    int mMinFontSize = 22;

    // 最大字体大小
    int mMaxFontSize = 100;
    ...
    // 贝塞尔曲线单侧宽度
    float mMaxBezierWidth = 300.0f;

    // 贝塞尔曲线单侧模拟线量
    int mMaxBezierLines = 32;

    // 列表字符颜色
    int mFontColor = 0xffffffff;  //白色
...
//	int  mTipFontColor = 0xff3399ff;
    ...
    public String[] ConstChar = {"A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"
    ...
    float mLastOffset[]; // 记录每一个字母的x方向偏移量, 数字<=0
...
//				mScroller.startScroll(0, 0, 255, 0, 1000);
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        //paddingTop = DisplayUtils.convertDIP2PX(mContext,20);
        ...
        mPopText = new TextView(getContext());
        mPopText.setBackgroundColor(Color.GRAY);
        mPopText.setTextColor(Color.CYAN);
        mPopText.setTextSize(50);
        mPopText.setGravity(Gravity.CENTER_HORIZONTAL
                | Gravity.CENTER_VERTICAL);
        popupWindow = new PopupWindow(mPopText, 100, 100);
        handler = new Handler();
    }
    ...
    public void setParams(String[] constChar, ViewPager viewPager, int top, int bottom) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 单个字母高度
        int length = 0;
        if (ConstChar.length > 26) {
            length = ConstChar.length / 2;
        } else {
            length = ConstChar.length;
        }
        float singleHeight = height / (float) length;
        ...
        for (int i = 0; i < mConLength; i++) {
            ...
            //Logs.v("hjz", "fontSize=" + DisplayUtils.px2sp(mContext, fontSize));
            mPaint.setTextSize(fontSize);
            float offsetX = ajustXPosAnimation(i, yPos);
            if (offsetX < 0) {
                mAlpha = 255 - fontSize * 5 / 2;
                mPaint.setColor(Color.parseColor("#2a8de9"));
            ...
            // 绘制字母
            drawTextInCenter(canvas, ConstChar[i], xPos + offsetX, yPos);

            // 如果手指抬起
            if (isUp) {
                if (mChooseIndex > -1) {
                    //Logs.v("hjz", "选中的字母下标mChooseIndex=" + mChooseIndex);
    ...
    private float ajustXPosAnimation(int i, float yPos) {
        ...
        if (this.mAnimating || this.mHideAnimation) {
            ...
            // 字母绘制时向左偏移量 进行修正, offset需要是<=0的值【如果需要在X轴上有偏移量的话，这个条件就要打开】
            /*float xPos = mTouch.x;      // 当前触摸的x方向位置
            float width = getWidth() - mWidthOffset;  // 整个屏幕的宽度 减去 字母的 宽度
            width = width - 60;
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (this.getWidth() > mWidthOffset) {
                    if (event.getX() < this.getWidth() - mWidthOffset - convertDIP2PX(mContext, 5))
                        return false;
                }
...
//                if (mViewPager != null) {
//                    mViewPager.setCanSilde(false);  //禁止 viewpager 在 字母 滑动列 向右滑动
//                }
                ...
                if (oldmChooseIndex != c && listener != null) {
                    if (c > 0 && c < ConstChar.length) {
                        Log.e("touch :",ConstChar[c] + "  "+c);
                        listener.onTouchLetterChanged(ConstChar[c], c);
                        curentIndex = ConstChar[c];
                        mChooseIndex = c;
                    }
                }
                showPopupWindow();
...
//                if (mViewPager != null) {
//                    mViewPager.setCanSilde(true);    //在 字母 滑动列，松开手指，恢复viewpager 滑动事件
//                }
                mTouch.x = event.getX();
                mTouch.y = event.getY();
