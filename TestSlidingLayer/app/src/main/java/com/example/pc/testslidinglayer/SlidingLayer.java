...
public class SlidingLayer extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        try {
			...
			case MotionEvent.ACTION_DOWN:
				...
				mLastX = mInitialX = ev.getRawX();//MotionEventCompat.getX(ev, mActivePointerId);
				mLastY = ev.getRawY();//MotionEventCompat.getY(ev, mActivePointerId);
    ...
    private int getScreenSideAuto(int newLeft, int newRight) {
        ...
        if (mScreenSide == STICK_TO_AUTO) {
            ...
            if (boundToLeftBorder == boundToRightBorder && getLayoutParams().width == android.view.ViewGroup.LayoutParams.MATCH_PARENT) {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
    	if (mOnScrollListener != null) {
    		int current = l + getWidth();
    		if (Math.abs(current - getWidth()) <= 2) {
				current = getWidth();
			} else if (current <= 2) {
				current = 0;
			}
			mOnScrollListener.OnScroll((float)current / (float) getWidth());
		}
    }
}
